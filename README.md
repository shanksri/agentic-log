...
evaluation REST API. Most of that later work exists as tested library code or REST endpoints not
yet wired into the primary search/agent routes described below — see the "Status" note in
...
- Agents — a single-shot investigation agent shipped early on; a separate four-agent framework
  (planner, hypothesis generation, critic, iterative orchestrator) has since been built as library
  code (doc 19) but is not wired into any API route.
